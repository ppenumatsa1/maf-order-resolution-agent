...
first_payload="$(jq -cn --arg input "Resolve delayed order ORD-1001" '{input: $input, metadata: {trace_evaluation_record_content: true}}')"
...
second_payload="$(jq -cn --arg input "Why was that resolution selected?" '{input: $input, metadata: {trace_evaluation_record_content: true}}')"
...
high_risk_payload="$(jq -cn --arg input "Resolve delayed order ORD-1009" '{input: $input, metadata: {trace_evaluation_record_content: true}}')"
...
high_risk_resume_payload="$(jq -cn --arg input "Approve" --arg checkpoint "$HIGH_RISK_CHECKPOINT" '{input: $input, decision: "approve", checkpoint_id: $checkpoint, metadata: {trace_evaluation_record_content: true}}')"
...
damaged_payload="$(jq -cn --arg input "Customer reports a damaged item for order ORD-1001 and requests a replacement" '{input: $input, metadata: {trace_evaluation_record_content: true}}')"
...
damaged_resume_payload="$(jq -cn --arg input "Approve damaged-item replacement" --arg checkpoint "$DAMAGED_CHECKPOINT" '{input: $input, decision: "approve", checkpoint_id: $checkpoint, metadata: {trace_evaluation_record_content: true}}')"
