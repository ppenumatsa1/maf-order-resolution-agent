...
@pytest.mark.asyncio
async def test_responses_client_maps_loopback_workflow_to_existing_thread() -> None:
...
@pytest.mark.asyncio
async def test_responses_client_reuses_a_supplied_foundry_conversation() -> None:
    payloads: list[dict[str, Any]] = []
    ...
    conversation_id = await client.start_workflow(
        thread_id="conv_existing",
        message="Why was that resolution selected?",
        create_conversation=False,
    )

    assert conversation_id == "conv_existing"
    assert payloads == [
        {
            "conversation": {"id": "conv_existing"},
            "input": "Why was that resolution selected?",
        }
    ]
